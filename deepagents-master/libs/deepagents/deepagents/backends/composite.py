...
class CompositeBackend:
    ...
    def _get_route_prefix(self, file_path: str, stripped_key: str) -> str:
        """Calculate the route prefix that was stripped from the file path.

        Args:
            file_path: Original file path (e.g., "/memory/notes.txt")
            stripped_key: Key after prefix removal (e.g., "/notes.txt")

        Returns:
            The route prefix without trailing slash (e.g., "/memory")
        """
        if stripped_key == "/":
            return file_path.rstrip("/")
        # file_path = "/memory/notes.txt", stripped_key = "/notes.txt"
        # prefix = "/memory"
        if len(stripped_key) > 1:
            return file_path[: -len(stripped_key) + 1]
        return ""
        ...
        if res.files_update:
            try:
                ...
                if runtime is not None:
                    ...
                    # Re-map keys to use original paths with route prefix
                    prefix = self._get_route_prefix(file_path, stripped_key)
                    for key, value in res.files_update.items():
                        if prefix and key.startswith("/"):
                            original_key = prefix + key[1:]
                        else:
                            original_key = key
                        files[original_key] = value
                    state["files"] = files
            except Exception:
                pass
        return res
